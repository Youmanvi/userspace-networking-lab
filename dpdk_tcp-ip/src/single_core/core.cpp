...
{
	cout << "CPU 0 mtcp thread running" << endl;

	
	mtcp_manager_t mtcp = ctx->mtcp_manager;
	int i=0;
	int recv_cnt;
	int rx_inf=0, tx_inf=0;
	...
	ts = ts_prev = 0;
	//bool print_dpdk = false;
	while ((!ctx->done || mtcp->flow_cnt) && !ctx->exit) {


		//cout << "Starting loop" << endl;
		//dpdkuse_ins.rx_proc_st = clock();
		...
		//for (rx_inf = 0; rx_inf < CONFIG.eths_num; rx_inf++) {
			...
			//TODO: Here logic became somewhat different than original. I am trying to receive only one packet from interface at a given time.
			// This will make receiver bottleneck. Ask Rahul about burst packet read, as original logic is. (Added by Jayesh).
			uint16_t len;
			//dpdkuse_ins.rx_dpdk = clock();
			recv_cnt = mtcp->iom->recv_pkts(ctx, rx_inf);
			
			//dpdkuse_ins.ex_st = clock();
			for(int i=0;i<recv_cnt;i++)
			{	
				struct rte_mbuf *parent;
				//cout << "Starting 1" << CONFIG.eths_num << endl;
				parent = (void *)mtcp->iom->get_rptr(mtcp->ctx, rx_inf, i, &len);
				if (parent != NULL)
				{
					void *temp_ptr = (void *)rte_pktmbuf_mtod_offset(parent, char *, ETHERNET_HEADER_LEN);
					
					ret = ProcessPacket(mtcp, rx_inf, ts, temp_ptr, len);
					//print_dpdk = true;
					//cout << "Received packet" << ret << endl;
					rte_pktmbuf_free(parent);
					//netmapuse.syncbufrx(ctx->cpu);
				}
				else
					mtcp->nstat.rx_errors[rx_inf]++;
			}
			//dpdkuse_ins.rx_app = clock();
			/*
			recv_cnt = mtcp->iom->recv_pkts(ctx, rx_inf);
		...
		//TODO: I don't think these two are doing anything. J.
		mtcp->iom->select(ctx);
		...
		//dpdkuse_ins.rx_proc_end = clock();
		/*if(print_dpdk){
			printf("===============>rx_proc_time:%lf\n",(float)((dpdkuse_ins.rx_proc_end - dpdkuse_ins.rx_proc_st)/(float)CLOCKS_PER_SEC));
			//printf("--------------->ex_time:%lf\n",(float)((dpdkuse_ins.ex_end - dpdkuse_ins.ex_st)/(float)CLOCKS_PER_SEC));
			printf("--------------->rx_time:%lf\n",(float)((dpdkuse_ins.rx_app - dpdkuse_ins.rx_dpdk)/(float)CLOCKS_PER_SEC));
			printf("--------------->tx_time:%lf\n",(float)((dpdkuse_ins.tx_dpdk - dpdkuse_ins.tx_app)/(float)CLOCKS_PER_SEC));
			print_dpdk = false;
		}*/
	}
