...
netmap_init_handle(struct mtcp_thread_context *ctxt)
{
	...
	//some function to from netmap_api to return netmap_desc. On second thoughts, no need of this.
	//npc->local_nmd[0] = nm_open(nifname, &base_nmd, 0, NULL);
}
/*----------------------------------------------------------------------------*/
int
...
netmap_get_wptr(struct mtcp_thread_context *ctxt, int nif, uint16_t pktsize)
{
	...
	return (uint8_t *) netmapuse.get_buffer_tx(ctxt->cpu);
...
{
	int len_here;
	return (uint8_t *) netmapuse.get_buffer_rx(ctxt->cpu,len_here);
	*len = (uint16_t *) len_here;
	/*
	struct netmap_private_context *npc;
	npc = (struct netmap_private_context *)ctxt->io_private_context;
...
/*----------------------------------------------------------------------------*/
//struct io_module_func netmap_module_func;
//netmap_module_func.load_module = netmap_load_module;
